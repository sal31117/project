...
if res.status_code == 200:  # HTTP 요청 성공 시
    try:
        ...
        # PM10과 PM2.5 시간대별 라인 차트
        ax.plot(df['dataTime'], df['pm10Value'], label="PM10", color='blue', marker='o')
        ax.plot(df['dataTime'], df['pm25Value'], label="PM2.5", color='orange', marker='o')
